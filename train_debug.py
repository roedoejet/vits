...
def main_debug():
    ...
    hps = utils.get_hparams()
    run(0, n_gpus, hps, debug=True, cpu_only=False)


def run(rank, n_gpus, hps, debug=False, cpu_only=False):
    global global_step
    if rank == 0:
...
def train_and_evaluate(
    rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers, debug, cpu_only
):
    net_g, net_d = nets
    ...
    for batch_idx, (
        ...
        pitch_predictions = pitch_predictions * x_mask
        pitch_targets = pitch_targets * x_mask

        energy_predictions = energy_predictions * x_mask
        energy_targets = energy_targets * x_mask

        log_duration_predictions = log_duration_predictions * x_mask
        log_duration_targets = log_duration_targets * x_mask
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                evaluate(hps, net_g, eval_loader, writer_eval, debug=debug, cpu_only=cpu_only)
...
def evaluate(hps, generator, eval_loader, writer_eval, debug=False, cpu_only=False):
    generator.eval()
    with torch.no_grad():
        for batch_idx, (
            x,
        ...
        if not debug:
            ...
            # DONE: cpu_only maybe isn't the right thing here - this is more about distributed models
            y_hat, mask, *_ = generator.infer(x, x_lengths, max_len=1000)
